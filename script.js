...
if (resultSearchBtn) {
  resultSearchBtn.addEventListener("click", async function () {
    const otpInputs = document.querySelectorAll(".otp input");
    const rollNumber = Array.from(otpInputs)
      .map((input) => input.value)
      .join("");

    const resultContainer = document.querySelector(".res");
    const errorMessage = document.querySelector(".result-error-message");
    ...
    if (rollNumber.length !== 5) {
      alert("Please enter a 5-digit roll number.");
      return;
    }
    ...
    try {
      ...
      if (studentResult) {
        document.getElementById("result-name").textContent = studentResult.name;
        document.getElementById("result-father-name").textContent = studentResult["Father name"];
        document.getElementById("result-roll-no").textContent = studentResult["roll no"];
        document.getElementById("result-status").textContent = studentResult.status;
      ...
      console.error("Error fetching or processing result data:", error);
      if (errorMessage) {
        errorMessage.textContent = "An error occurred while checking the result.";
